...
export function EmailCodeSignUp({ onSwitchMode }: { onSwitchMode: () => void }) {
  ...
  async function submitCode(codeToSubmit: string) {
    ...
    try {
      const result = await signUp.attemptEmailAddressVerification({ code: codeToSubmit })
        ...
        <form className="flex flex-col gap-5" onSubmit={handleCodeSubmit}>
          <div className="text-center">
            ...
            <p className="mt-1.5 text-xs text-gray-500 max-w-[320px] mx-auto leading-relaxed">
              ...
              <span className="font-medium text-gray-800 break-all">{email}</span> to finish creating your account.
            </p>
          </div>
